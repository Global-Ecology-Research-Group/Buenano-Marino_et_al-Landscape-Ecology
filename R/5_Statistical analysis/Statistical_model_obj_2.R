...
city_rel <- all_city_data %>%
  filter(complete.cases(mean_h),
         complete.cases(NDVI)) %>%
  select(total_richness, duration, mean_h, built,
...
corr_mat <- cor(city_rel_corr[,-c(1, 11:13)], method="pearson")
corr_mat

# visualize the correlation
#jpeg("Results/predictor_correlation.jpeg", width = 7, height = 7, units = "in", res = 300)
...
model_test <- gam(total_richness ~ built + mean_h + 
...
# check histogram of building height
hist(unique(all_city_data$mean_h))

building_height <- data.frame(num=1:length(unique(all_city_data$mean_h)), mean_h=unique(all_city_data$mean_h))

ggplot(building_height, aes(x = mean_h)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.7) +  # Smaller binwidth for more detail
  scale_x_continuous(
    name = "Number of Stories",  # Change x-axis label
    breaks = seq(0, max(all_city_data$mean_h, na.rm = TRUE), by = 2),  # More frequent breaks
    labels = function(x) round(x / 2.74, 1)  # Convert meters to stories
  ) +
  ...
  labs(
    title = "Building Heights in Stories",
    y = "Frequency"
  )

ggsave("Figures_and_Tables/Figure_A6.jpeg", width = 10, height = 8, units = "in")

# let's check out the 95% quantile
quantile(unique(all_city_data$mean_h), p=c(0.05, 0.95), na.rm=TRUE)
...
city_data$mean_h <- scale(city_data$mean_h)
...
analyze_city <- function(file_path) {
  tryCatch({
    ...
    # New model
    new_model <- gam(total_richness ~ built + mean_h + 
                       bare + grass + shrub_and_scrub + water + trees + NDVI +
...
# save the results
saveRDS(results, "Intermediate_data/model_results/Obj2_Models.RDS")
...
ggsave("Figures_and_Tables/Figure_A10.png", 
...
ggsave("Figures_and_Tables/Figure_4.png", 
       width = 10, height = 5, units = "in", dpi = 300)
...
ggsave("Figures_and_Tables/Figure_5.png", 
